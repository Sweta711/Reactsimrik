...
const Jeep = () => {
const [expandedJeepId, setExpandedJeepId] = useState(jeeps[0].id);
...
e.preventDefault();
if (!bookingJeep) return;

const daysNumber = Number(form.days) > 0 ? Number(form.days) : 1;
const total = bookingJeep.price * daysNumber;

alert(
    `Booking confirmed!\n\n` +
    `Vehicle: ${bookingJeep.name}\n` +
    `Name: ${form.fullName}\n` +
    `Email: ${form.email}\n` +
    `Phone: ${form.phone}\n` +
    `Days: ${daysNumber}\n` +
    ...
    <div className="max-w-6xl mx-auto flex flex-col lg:flex-row items-start gap-4 md:gap-6">
        ...
        >
            ...
            <div>
                ...
                Choose a jeep and then book your seats.
                </p>
            </div>
            <span
            ...
            >
                ...
                {filteredJeeps.length} jeeps found
            </span>
            </div>
            ...
            {filteredJeeps.map((jeep) => (
                ...
                >
                    ...
                    const active =
                        expandedJeepId === jeep.id && expandedTab === tab.key;
                    return (
                        <button
                        key={tab.key}
                ...
                {expandedJeepId === jeep.id && (
                    ...
                    {expandedTab === "gallery" &&
                        (jeep.gallery && jeep.gallery.length ? (
                        <>
                            {expandedImage &&
                            expandedImage.vehicleId === jeep.id && (
                                <div
                            ...
                            {jeep.gallery.map((img, index) => (
                                <img
                                key={index}
                                src={img}
                                alt={`${jeep.name} interior ${index + 1}`}
            ...
            {!filteredJeeps.length && (
                <div className="text-center text-sm text-[#6b7280] py-10">
                No jeeps match your filters. Try changing your options.
                </div>
            )}
            </div>
        </section>
    ...
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-[rgba(15,23,42,0.55)] px-3">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="text-[#374151]">
                <div>
                Vehicle: <strong>{bookingJeep.name}</strong>
                </div>
                <div>
                Price per person: <strong>Rs. {bookingJeep.price}</strong>
                </div>
            </div>
            <button
                type="submit"
